      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {data.length === 0 ? (
          <div className="col-span-full rounded-md border border-card-border bg-card-bg flex flex-col items-center justify-center py-24 text-foreground-secondary space-y-4">
            ...
            <p className="text-sm font-medium">Belum ada testimoni. Tambahkan yang pertama!</p>
          </div>
        ) : (
          data.map((item) => (
            <div
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-xs text-foreground-secondary italic line-clamp-4 leading-relaxed">
                  &ldquo;{item.text.id || item.text.en || item.text.ms}&rdquo;
                </p>
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <Badge variant="outline" className="text-[9px] py-0 px-1 rounded-sm border-primary-500/20 text-primary-600">ID</Badge>
                  <Badge variant="outline" className="text-[9px] py-0 px-1 rounded-sm border-blue-500/20 text-blue-600">EN</Badge>
                  <Badge variant="outline" className="text-[9px] py-0 px-1 rounded-sm border-green-500/20 text-green-600">MS</Badge>
                </div>
              </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-4xl p-0 overflow-hidden border-none rounded-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Nama Pelanggan</Label>
                  <Input 
                    value={form.name} 
                ...
                <ImageUpload
                  label="Foto Profil"
                  helperText="Rasio 1:1 direkomendasikan."
                  aspectRatio={1}
                  value={form.avatar}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                     ...
                     <div className="flex items-center gap-2 h-9 px-3 border border-input rounded-md bg-background">
                        ...
                        />
                        <span className="text-[10px] font-medium uppercase">{form.isPublished ? "Public" : "Draft"}</span>
                     </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-1">
                  <Label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Lokalisasi Konten</Label>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                  ...
                  {(['en', 'id', 'ms'] as const).map((lang) => {
                    ...
                    return (
                      <TabsContent key={lang} value={lang} className="mt-4 space-y-4 animate-in fade-in-50 duration-200">
                        <div className="space-y-1.5">
                          <Label className="text-[10px] text-muted-foreground uppercase font-bold">Jabatan ({lang.toUpperCase()})</Label>
                          <Input 
                            value={form[roleKey] as string} 
                            onChange={(e) => f(roleKey, e.target.value)} 
                            placeholder="Contoh: Wisatawan Family"
                            className={`rounded-md ${lang === 'en' ? 'border-primary-200 bg-primary-50/10' : ''}`}
                          />
                        </div>
                        <div className="space-y-1.5">
                          <Label className="text-[10px] text-muted-foreground uppercase font-bold">Ulasan ({lang.toUpperCase()})</Label>
                          <Textarea 
                            rows={6} 
          ...
          <div className="flex justify-end gap-3 p-6 bg-muted/30 border-t border-card-border">
            <Button variant="ghost" onClick={() => setOpen(false)} className="rounded-md text-xs h-9">Batal</Button>
            <Button
              onClick={handleSubmit}
              disabled={isPending}
              className="bg-primary-500 hover:bg-primary-600 text-white min-w-[100px] rounded-md shadow-sm text-xs h-9"
            >
              {isPending ? (
              ...
              {editing ? "Simpan Perubahan" : "Simpan Testimoni"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
